...
void mkt::map_index_in_place(mkt::DArray<T>& a, Functor f){
	...
	for(int gpu = 0; gpu < 1; ++gpu){
		...
		dim3 dimBlock(1024);
		dim3 dimGrid((gpu_elements+dimBlock.x-1)/dimBlock.x);
		mkt::kernel::map_index_in_place<<<131072, dimBlock, smem_bytes, mkt::cuda_streams[gpu]>>>(devptr, gpu_elements, gpu_offset, f);
	}
}
...
template<>
void mkt::print<Float2>(std::ostringstream& stream, const Float2& a) {
  stream << "[";
  stream << "x: ";
...
template<>
void mkt::gather<float2>(mkt::DArray<float2>& in, mkt::DArray<float2>& out){
	in.update_self();
	std::copy(in.get_data(), in.get_data() + in.get_size_local(), out.get_data());
	out.update_devices();
}
