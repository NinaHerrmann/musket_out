...
//}		
		newa.x += (input.get_data_local(((Index) + ((j) * (channels)))) * coeff.get_data_local(((Index%(channels*taps)) + ((j) * (channels))))
				);
			}
			}
...
return newa;
	...
	int main(int argc, char** argv) {
		...
		timer.Start();
		mkt::DArray<float> input(0, 201340928, 201340928, 0.0f, 1, 0, 0, mkt::DIST, mkt::COPY);
          //      mkt::DArray<float> input_double(0, 201326592,201326592, 0.0f, 1, 0, 0, mkt::DIST, mkt::COPY);
